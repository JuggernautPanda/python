...
/*
    ...
    * 
    * Compile : gcc Server_admin.c -o Server_admin -lpthread -`mysql_config --cflags --libs`
*/
...
void *connection_handler(void *socket_desc)
{
    ...
    int i=0;
    char *message , client_message[2000], uname[2000],passwd[2000];
    ...
    if (read_size > 0)
    {
      ...
      for (i=0;i<strlen(client_message);i++)
      {
		  if (client_message[i] == ",")
		  {
			  if (count == 0)
			  {
				  strncpy(uname,client_message,i);
				  count = 1;
		      ...
		      {
				  strncpy(passwd,&client_message[i],strlen(client_message));
				  count = 2;
