...
def Login(uname,passwd):
	...
	#fieldNames=["Address"]
	#Addres = []
	#Address = multenterbox(msg,title, fieldNames)
	#msg="Select Flight and Date"
	#title = "Select Flight"
	#flightDate = ["AS400: 17th Nov NYK to WDC","AS500: 18th Nov WDC to California"]
...
while 1:
	...
	if choice=="Booking" :
		...
		title = "Login Page"
		fieldNames = ["UserName","Book for AS:400 17th Nov Enter YES to confirm"]
		fieldValues = []  # we start with blanks for the values
		...
		while 1:
			...
			if errmsg == "": break # no problems found
			fieldValues = multpasswordbox(errmsg, title, fieldNames, fieldValues)
		Login(fieldValues[0],fieldValues[1])
